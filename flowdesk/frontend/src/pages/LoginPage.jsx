...
const SceneBg = () => (
  <svg style={{position:'absolute',inset:0,width:'100%',height:'100%'}} viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
    <defs>
      ...
      <radialGradient id="a1" cx="25%" cy="30%" r="45%">
        <stop offset="0%" stopColor="#6366f1" stopOpacity="0.55"/>
        <stop offset="100%" stopColor="#6366f1" stopOpacity="0"/>
      </radialGradient>
      <radialGradient id="a2" cx="60%" cy="20%" r="40%">
        <stop offset="0%" stopColor="#14b8a6" stopOpacity="0.4"/>
        <stop offset="100%" stopColor="#14b8a6" stopOpacity="0"/>
      </radialGradient>
      <radialGradient id="a3" cx="80%" cy="40%" r="35%">
    ...
    <rect width="800" height="600" fill="url(#a1)" filter="url(#b8)"/>
    <rect width="800" height="600" fill="url(#a2)" filter="url(#b8)"/>
    <rect width="800" height="600" fill="url(#a3)" filter="url(#b8)"/>
    {[80,150,220,300,380,460,540,620,700,760,110,340,580].map((x,i) => (
      <circle key={i} cx={x} cy={[40,20,55,15,35,10,45,25,50,18,70,60,70][i]} r={[1,0.8,1,0.8,1,0.8,1,0.8,1,0.8,0.6,0.6,0.6][i]} fill="white" opacity={[0.7,0.5,0.6,0.4,0.7,0.5,0.6,0.4,0.7,0.5,0.4,0.3,0.4][i]}/>
    ))}
    <circle cx="180" cy="90" r="22" fill="url(#moon)" filter="url(#b2)"/>
    <circle cx="180" cy="90" r="13" fill="#dde4ff" opacity="0.9"/>
    ...
    <rect x="0" y="420" width="800" height="180" fill="url(#water)" opacity="0.9"/>
    <line x1="50" y1="440" x2="200" y2="440" stroke="rgba(99,102,241,0.15)" strokeWidth="1"/>
    <line x1="80" y1="460" x2="220" y2="460" stroke="rgba(20,184,166,0.12)" strokeWidth="0.8"/>
    <ellipse cx="180" cy="460" rx="8" ry="30" fill="#a5b4fc" opacity="0.1" filter="url(#b4)"/>
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, flex: 1, padding: '40px 48px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '100vh' }}>
        ...
        <div>
          <h2 style={{ color: 'white', fontSize: 36, fontWeight: 700, lineHeight: 1.25, marginBottom: 12, textShadow: '0 2px 20px rgba(0,0,0,0.8)' }}>
            Welcome back.<br />
            <span style={{ color: '#a5b4fc' }}>Sign in to continue.</span>
          </h2>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['Tasks', 'Projects', 'Team', 'Analytics'].map(tag => (
              <span key={tag} style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', color: 'rgba(165,180,252,0.9)', padding: '4px 12px', borderRadius: 999, fontSize: 12 }}>{tag}</span>
            ))}
          </div>
        </div>
      ...
      <div style={{ position: 'relative', zIndex: 2, width: 300, margin: '0 40px 0 0', background: 'rgba(10,12,25,0.75)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '28px 24px', backdropFilter: 'blur(20px)' }}>
        <h1 style={{ color: 'white', fontSize: 20, fontWeight: 700, margin: '0 0 4px' }}>Welcome back</h1>
        <p style={{ color: 'rgba(255,255,255,0.38)', fontSize: 12, margin: '0 0 20px' }}>Sign in to your FlowDesk account</p>
