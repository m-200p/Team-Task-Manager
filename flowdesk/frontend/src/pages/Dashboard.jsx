...
import { Plus, CheckCircle2, Clock, AlertTriangle, BarChart3, X, Save, Trash2, Lock, Calendar, User, ChevronRight, Zap, UserPlus } from 'lucide-react';
...
function getRiskInfo(project, allTasks) {
  ...
  // Check project-level deadline (use earliest task due date as proxy)
  ...
  if (highPending.length >= 2)
    reasons.push(`${highPending.length} high-priority tasks are still pending`);
  if (overdue.length > 0)
    reasons.push(`${overdue.length} overdue task${overdue.length > 1 ? 's' : ''} not completed`);

  const percent = total === 0 ? 0 : Math.round((done / total) * 100);
  return { isAtRisk: reasons.length > 0, reasons, percent, done, total };
}

// ─── AT RISK Badge with tooltip ─────────────────────────
    ...
    <div className="relative inline-block">
      ...
      {show && (
        <div className="absolute bottom-full left-0 mb-2 w-60 bg-ink-900 text-white text-xs rounded-xl p-3 shadow-panel z-20 leading-relaxed">
          <p className="font-semibold mb-2 text-amber-300 flex items-center gap-1">
            <AlertTriangle size={11} /> Risk Diagnostic
          </p>
      ...
      <div className="bg-white rounded-2xl shadow-panel w-full max-w-sm animate-fade-up">
        <div className="flex items-center justify-between p-5 border-b border-surface-100">
          <h2 className="font-semibold text-ink-900 flex items-center gap-2">
            <UserPlus size={16} className="text-brand-500" /> Add Member to Project
          </h2>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            {allUsers.length === 0
              ? <p className="text-sm text-ink-400 bg-surface-50 rounded-xl p-3">No more users to add. Create from Admin Panel first.</p>
                    ...
                    <div key={p._id} onClick={() => setActiveProject(p)}
                      ...
                      <div className="flex items-center justify-between pl-0.5">
                        {risk
                          ...
                          : <span className="text-[10px] text-ink-300">{percent}% done</span>
                        }
                  ...
                  return (
                    <div key={col.id} className="flex flex-col min-h-0">
                      ...
                      <div className="flex flex-col gap-2 overflow-y-auto flex-1 pr-0.5">
                        {colTasks.map(t => {
                          ...
                          return (
                            <div key={t._id} onClick={() => setSelectedTask(t)}
                              className="bg-white border border-surface-200 rounded-xl p-3 cursor-pointer hover:border-brand-300 hover:shadow-card transition-all duration-150 group">
                        ...
                        {colTasks.length === 0 && (
                          <div className="border-2 border-dashed border-surface-200 rounded-xl h-16 flex items-center justify-center text-[10px] text-ink-200">Empty</div>
                        )}
                      </div>
                    </div>
                  );
